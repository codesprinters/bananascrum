...
class SeleniumTestCase < ActionController::IntegrationTest 
  ...
  def login(login = "admin_1", password = "password")
    type "login", "#{login}"
  ...
  def expand_item(user_story = nil)
    ...
    item_path = "//div[@class=\'expandable-list\']"+ path +"//div[@class=\'icon expand-icon expand\']"
    click item_path 
  end

  def edit_task_summary(task_path, new_value)
    click task_path + "//span[@class='task-summary highlight']"
    wait_for 'isElementPresent("//form[@class=\'editor-field\']/input[@type=\'text\']")'
    type "//form[@class='editor-field']//input[@type='text']", new_value
    click "//button[@type='submit']"
  end

  def edit_task_estimate(task_path, new_value)
    #    TODO Consider use that code below, and refactor tests if it is good idea.
    #
    #    DomainChecks.disable do
    ...
    #    click task_path + "//span[@class='task-estimate highlight']"
    #    type "//form[@class='editor-field']/input[@type='text']", new_value
    #    click "//button[@type='submit']"
    #    sleep 2
  ...
  def assert_task(task_id, summary, estimate, users)
    wait_for "getText('#{task_id}').normalize() == '#{estimate} [#{users}] #{summary}'"
  end
