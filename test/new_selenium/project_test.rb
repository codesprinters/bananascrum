...
class ProjectTest < SeleniumTestCase
  ...
  def test_edit_values_in_project
    ...
    wait_for 'getText("//li[@id=\'item-2\']//div[@class=\'item-estimate-container\']").normalize() == "[1 LOL]"'
    wait_for 'isTextPresent("total LOL")'

    click_and_wait "link=Sprint"
    select "filter", "label=All"
    wait_for 'getText("//li[@id=\'item-23\']//div[@class=\'item-estimate-container\']").normalize() == "[1 LOL]"'
    wait_for 'getText("//li[@id=\'item-23\']//div[@class=\'total-tasks-effort\']").normalize() == "(0parsec)"'


    click_and_wait "link=Sprint"
    click_and_wait "link=Plan"
    wait_for 'isTextPresent("total LOL")'
  end
...
Are sure you want to do this[\s\S]$/ =~ get_confirmation
  ...
  def test_edit_used_estimates
    ...
    click "//div[@id='settings']//input[@type='submit']"
    sleep 2
    select "project_id", "label=Project 1"
    wait_for_page_to_load("3000");
    click_and_wait "link=Sprint"
    click "//li[@id='item-19']//span[contains(@class, 'item-estimate')]"
    select "//div[@class='item-estimate-container']//select", "label=15"
    ...
    click_and_wait "link=Sprint"
    click "//li[@id='item-19']//span[contains(@class, 'item-estimate')]"
    select "//div[@class='item-estimate-container']//select", "label=81"
    ...
    click_and_wait "link=Sprint"
    click "//li[@id='item-19']//span[contains(@class, 'item-estimate')]"
    select "//div[@class='item-estimate-container']//select", "label=0.5"
    select "filter", "label=All"
  end
  ...
  def test_reset_project_settings
    ...
    click "//li[@id='item-19']//span[contains(@class, 'item-estimate')]"
    select "//div[@class='item-estimate-container']//select", "label=0.5"
    wait_for 'isTextPresent("(29.5 SP)")'
    wait_for 'isTextPresent("(40 h)")'
  end
