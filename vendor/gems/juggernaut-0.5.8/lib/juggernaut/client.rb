...
module Juggernaut
  class Client
    ...
    class << self
      ...
      def disconnect_clients_with_flag_set
        @@clients.each do |client|
          if client.should_disconnect
            client.connections.each { |conn| conn.close_connection }
          end
        end
      end
    ...
    def subscription_request(channels, &callback)
      return true unless options[:subscription_url]
      post_request(options[:subscription_url], channels, { :timeout => (options[:post_request_timeout] || 5) }, &callback)
    end
    ...
    def post_request(url, channels = [ ], options = { })
      ...
      subscribed_clients = Juggernaut::Client.find_by_channels(channels)
      i = -1
      params.push(*subscribed_clients.map { |c| "clients_in_channel[#{i += 1}]=#{c.session_id}" })
      
      if block_given?
        @server.nonblocking_post(uri, params.join('&')) do |a|
          yield(a) 
        end
      else
        @server.nonblocking_post(uri, params.join('&'))
      end
    end  
