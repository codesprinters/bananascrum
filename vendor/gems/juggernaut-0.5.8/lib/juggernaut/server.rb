...
module Juggernaut
  class Server < EventMachine::Connection
    ...
    def nonblocking_post(uri, params)
      logger.debug("Sending request to #{uri.inspect} with params '#{params}'")
      ...
      http.errback do
        logger.debug "Sending request to the server failed. Disconnecting client #{@client.friendly_id}"
        @client.should_disconnect = true
      end
      
    rescue EventMachine::ConnectionError => e
      logger.error("#{e.to_s}. Propably limit of open files (sockets) per process reached.")
    end
    ...
    protected
      ...
      def subscribe_command
        ...
        @client.subscription_request(@channels) do |response|
          logger.debug "SUBSCRIBE callback: response status #{response[:status]}"
          unless response[:status] == 200
            logger.debug "Disconnecting client #{@client.friendly_id}"
            close_connection
          else
            begin
              ...
              logger.error "JSON parse error, response content: #{response[:content]}"
            end
          end
        end
      end
