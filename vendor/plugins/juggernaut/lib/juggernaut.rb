...
module Juggernaut
  begin
    CONFIG = YAML::load(ERB.new(IO.read("#{RAILS_ROOT}/config/livesync_hosts.yml")).result).freeze
  rescue
    Kernel.warn("#{RAILS_ROOT}/config/juggernaut_hosts.yml not found!")
    CONFIG = { :hosts => {} }.freeze
  end
  CR = "\0"
  ...
  class << self
    ...
    def send_data(hash, response = false)
      ...
      hosts.each do |address|
        begin
          ...
          ApplicationController.logger.debug("connecting to #{address.inspect}")
          socket = nil
          Timeout::timeout(5, false) {
    ...
    def hosts
      CONFIG[:hosts].select {|h| 
        !h[:environment] or h[:environment].to_s == ENV['RAILS_ENV'].to_s
      }
    end
