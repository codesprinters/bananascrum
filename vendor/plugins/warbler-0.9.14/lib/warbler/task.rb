    ...
    def define_single_gem_tasks(gem_pattern, targets, version = nil)
      ...
      unless spec.platform.nil? || spec.platform == Gem::Platform::RUBY
        [spec.platform, spec.original_platform].each do |p|
          name = "#{gem_name}-#{p}"
          if File.exist?(File.join(Gem.dir, 'cache', "#{name}.gem")) or File.exist?(File.join(config.rails_root, 'vendor', 'gems', name))
            gem_name = name
            break
          end
        end
      ...
      vendor_src = File.join(config.rails_root, 'vendor', 'gems', gem_name)
      ...
      task gem_unpack_task_name => [dest.pathmap("%d")] do |t|
        if File.exists?(vendor_src)
          sh "cp -Rf #{vendor_src} #{dest}"
        else
