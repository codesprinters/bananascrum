  ...
  class JsRoutes
    class << self
      ...
      def build_params segs, others = ''
        s = []
        segs.each do |seg|
          if seg.is_a?(ActionController::Routing::DynamicSegment) && seg.key != :format
            s << seg.key.to_s.gsub(':', '')
          end
        end
        s << 'verb'
      ...
      def build_path segs
        ...
        segs.each_index do |i|
          ...
          elsif seg.is_a?(ActionController::Routing::DynamicSegment) && seg.key != :format
            s << "' + #{seg.key.to_s.gsub(':', '')} + '"
          end
        end
        s
...
# s << route.inspect# if route.instance_variable_get(:@conditions)[:method] == :put
          s << "/////\n//#{route}\n" if @@debug
          s << <<-JS
function #{name.camelize(:lower)}Path(#{build_params route.segments}){ return bs._rootPath + '#{build_path route.segments}';}
JS
